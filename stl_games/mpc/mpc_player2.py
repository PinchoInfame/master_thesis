...
class MPCPlayer2:
    ...
    def build(self, x0_2):
        ...
        for j in range(len(self.goal_list)):
            h0_goal1 = self.goal_area_size - (x0_2[0]-self.goal_list[j][0])
            h0_goal2 = self.goal_area_size - (x0_2[1]-self.goal_list[j][1])
            h0_goal3 = self.goal_area_size - (-x0_2[0]+self.goal_list[j][0])
            h0_goal4 = self.goal_area_size - (-x0_2[1]+self.goal_list[j][1])
        ...
        for j, obs in enumerate(self.obs_list):
            cbf_constraint = self.parameter1_obs[(j*2):(j*2)+2] @ (self.u2[0:2, 0]) + self.parameter2_obs[j]
        ...
        for k in range(self.horizon-1):
            ...
            for j in range(len(self.goal_list)):
                dist_to_goal1 = self.x2[0,k] - self.goal_list[j][0]
                dist_to_goal2 = self.x2[1,k] - self.goal_list[j][1]
                dist_to_goal3 = -self.x2[0,k] + self.goal_list[j][0]
                dist_to_goal4 = -self.x2[1,k] + self.goal_list[j][1]
                ...
                dist_to_goal1_next = self.x2[0,k+1] - self.goal_list[j][0]
                dist_to_goal2_next = self.x2[1,k+1] - self.goal_list[j][1]
                dist_to_goal3_next = -self.x2[0,k+1] + self.goal_list[j][0]
                dist_to_goal4_next = -self.x2[1,k+1] + self.goal_list[j][1]
        ...
        for j in range(len(self.goal_list)):
            final_dist_to_goal1 = self.x2[0, self.horizon] - self.goal_list[j][0]
            final_dist_to_goal2 = self.x2[1, self.horizon] - self.goal_list[j][1]
            final_dist_to_goal3 = -self.x2[0, self.horizon] + self.goal_list[j][0]
            final_dist_to_goal4 = -self.x2[1, self.horizon] + self.goal_list[j][1]
    ...
    def solve(self, x2_ref, x1_fixed, x0_2, current_iteration=0):
        ...
        self.parameter2_coll.value = db_dx @ ((Ap @ x0_2 - x0_2[0:2]) / self.dt) + self.alpha_coll * b
    ...
    def solve_receding_horizon(self, x2_ref, x1_fixed, x0_2, max_iterations=10, current_iteration=0):
        x2_opt = [x0_2]
        u2_opt = []
        x_current = x0_2
        for t in range(max_iterations):
            u_opt, x_prev, u_prev = self.solve(x2_ref, x1_fixed, x_current, current_iteration)
            if u_opt is None:
                print("Solver failed")
                exit()
